...
public class CaptainCardDAO {
    ...
    public List<CaptainCardCO> getAll() {
        String sql = "select * from captain_card ";
        return j.query(sql, new Object[]{}, new RowMapper<CaptainCardCO>() {
            @Override
            public CaptainCardCO mapRow(ResultSet rs, int rowNum) throws SQLException {
                CaptainCardCO cc = new CaptainCardCO();
    ...
    public CaptainCardCO get(long id) {
        String sql = "select * from captain_card where id=?";
        return j.queryForObject(sql, new Object[]{id}, new RowMapper<CaptainCardCO>() {
            @Override
            public CaptainCardCO mapRow(ResultSet rs, int rowNum) throws SQLException {
                CaptainCardCO cc = new CaptainCardCO();
                cc.setId(rs.getInt(1));
    ...
    public List<CaptainCardCO> getByCaptainId(long captainId) {
        String sql = "select * from captain_card where captain_id=?";
        return j.query(sql, new Object[]{captainId}, new RowMapper<CaptainCardCO>() {
            @Override
            public CaptainCardCO mapRow(ResultSet rs, int rowNum) throws SQLException {
                CaptainCardCO cc = new CaptainCardCO();
                cc.setId(rs.getInt(1));
